...
class DemographicProfile:
    ...
    def filter_condition(self, df: pd.DataFrame) -> pd.Series:
        ...
        country_condition = self.country.filter_condition(df) if self.country is not None else default
        age_condition = self.age.filter_condition(df) if self.age is not None else default
        sex_condition = self.sex.filter_condition(df) if self.sex is not None else default
